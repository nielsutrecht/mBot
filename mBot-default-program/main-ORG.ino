...
boolean onCrossroads() {
  uint8_t val = line.readSensors();
  return (val == S1_IN_S2_IN);
}

boolean onRoad() {
  uint8_t val = line.readSensors();
  return (val == S1_OUT_S2_OUT);
}

boolean needsSmallLeft() {
   uint8_t val = line.readSensors();
  return (val == S1_IN_S2_OUT);
}

boolean needsSmallRight() {
   uint8_t val = line.readSensors();
  return (val == S1_OUT_S2_IN);
}
